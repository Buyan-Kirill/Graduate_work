...
Важно: test ROC AUC `0.949385` нельзя считать доказанным приростом нового
рецепта относительно исторического DeiT, потому что эти числа получены на
разных test-выборках и разными процедурами оценки. Для разделения эффекта
данных и рецепта подготовлено отдельное контролируемое сравнение legacy
`try_1` и current DeiT на одном manifest:
